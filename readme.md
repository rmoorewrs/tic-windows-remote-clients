...
__version 0.2, Mar 2, 2018 -- richard.moore@windriver.com__

The files in this repo can install the Titanium remote clients to your Windows or Mac Machine (see requirements). An uninstall batch file is also provided.
> Note: the latest openstack clients from the pypi repository will be installed, except for the Wind River system/cgts client which is located in this repo. Future updates to the pypi versions could break things. Most common operations have been tested to work, but Windows and Mac are not officially supported by Wind River.

## Requirements for Windows:

1. Python 2.x must be installed on your windows machine and in the exe PATH (REQUIRED): 
> Note: use the latest python 2.x (2.7.14 as of this writing) -- python 3.x may work, but hasn't been tested
...
## Uninstalling Remote Clients:

To uninstall the remote clients, run the `uninstall-windows-rcli.bat` file in a Windows CMD shell. You will be prompted to confirm uninstallation of each package.

----

## Mac Requirements

1. Python 2.7 and pip must be installed on your mac machine and be in the execution path (REQUIRED):
Recent versions of Mac OS should already come with python 2.7 installed. 
> Note: It's recommended to use python 2.7 -- versions of python 3 may or may not work.
Open a bash shell and check that python 2.7 and pip are installed:
```
$ python --version
Python 2.7.14
$ pip --version
pip 9.0.1
```
You should see the versions displayed as above. If this fails, install per the instructions on the python website.
`http://docs.python-guide.org/en/latest/starting/install/osx/`

2. Git must be installed on your machine and in the execution path (REQUIRED): 
Check for git by opening a bash shell and typing:
```
$ git --version
git version 2.16.2
```
You should see the version displayed as above. If this fails, install per the instructions on the git website.
`https://git-scm.com/book/en/v2/Getting-Started-Installing-Git`


3. You should have the latest copy of this repository, `tic-remote-clients-windows` on your machine in a convenient location, such as `~/git`.
- cd into your git directory and clone the repo, or do a `git pull` if you previously cloned it.
- uzip an archive if that's how you obtained the repo
...
## Remote Client Installation for Mac:
- open a bash shell in the top level install directory
- run the `install-mac-rcli.sh` script and wait for it to complete
```
$ sudo ./install_mac_rcli.sh
```
- the script should finish without errors, but if there errors, check for missing dependencies and try to install any reported missing packages.

### Test the remote clients
Open a bash shell, source the remote credentials shell you downloaded from Horizon and test the various clients with the `help` parameter
```
$ source myproj-openrc.sh
$ cinder help
$ glance help
$ heat help
$ neutron help
...
$ system help
```
## Uninstalling Remote Clients on Mac:
To uninstall the remote clients, run the `uninstall_mac_rcli.sh` file in a bash shell. You will be prompted to confirm uninstallation of each package.
```
$ sudo ./uninstall_mac_rcli.sh
```

## Known Issues:
- no error checking in the installation scripts
- Need directions for handling controllers behind a proxy (i.e. Hudson lab)
=======
# Titanium Remote CLI CLients for Windows and Mac
__version 0.3, May 31, 2019 -- rmoorewrs@gmail.com
...
>>>>>>> 736d99438dfffffc106044d0ca4911aa627c2605
