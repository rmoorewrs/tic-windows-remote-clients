...
def backend_modify(cc, args):
    ...
    # US111673: Because patchback procedure doesn't support DB upgrade,
    #      for US111673 R5 patchback we have to use storage_ceph DB table
    #      for both internal ceph and external ceph.
    #      For an external ceph backend, we will direct the request to 
    #      storage_ceph_external API
    if (backend == constants.SB_TYPE_CEPH and
         backend_entry.capabilities.get('location', None) == 'external'):
